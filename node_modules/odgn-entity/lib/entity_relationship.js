...
var initEntityER = function( entityDef, options ){
    ...
    // dlog('initEntityER for ' + entityDef.type );
    ...
    _.each( entityDef.ER, function(spec){
        ...
        if( spec.oneToMany ){
            ...
            // else{
            // console.log('found entity ' + spec.oneToMany + inspect(spec) );
            // }
            // print_ins( refEntity );
...
exports.oneToMany = function( entityDef, spec, options ){
    if( !spec  || !spec.type || entityDef === undefined ){
        console.log('could not find spec ' + JSON.stringify(spec) );
        return;
    }
    var debug = options && options.debug,
    ...
    // dlog('applying 1toM on ' + entityDef.type + ' to ' + JSON.stringify(spec) );
    ...
    // console.log('dammit ' + entityDef.type );
    // if( entityDef.type === 'channel' ) {
    ...
    entityDef.Entity.prototype.initialize = function(){
        ...
        // console.log('creating with ' + spec.create );
        var self = this,
