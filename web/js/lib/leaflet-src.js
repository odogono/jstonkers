...
// console.debug('min zoom ' + z1 + ' ' + z2 + ' ' + z3 );
...
// console.debug('getBoundsZoom ' + zoom + ' ' + boundsSize + ' ' + size );
			if (!inside) {
...
// console.debug('getBoundsZoom maxZoom ' + maxZoom + ' zoom ' + zoom + ' t.o.m '  + this.options.minZoom );
	...
	_resetView: function (center, zoom, preserveMapOffset, afterZoomAnim) {
		...
		// console.log('initial topleft ' + this._initialTopLeftPoint + ' ' + L.DomUtil.getPosition(this._mapPane) );
		// dmark( this._initialTopLeftPoint.x, this._initialTopLeftPoint.y );
		...
		// NOTE : added by AV
		if( this.options.maxBounds ){
			var panInside = function () {
				this.panInsideBounds(this.options.maxBounds);
			}
			L.Util.requestAnimFrame(panInside, this, true, this._container);
		}
	},
	...
	_getTopLeftPoint: function () {
		...
		// console.log('mapPane ' + mapPanePos );
	...
	_limitZoom: function (zoom) {
		var min = this.getMinZoom(),
			max = this.getMaxZoom();
// console.debug('_limitZoom ' + zoom + ' ' + min + '/' + max + ' limit to ' + Math.max(min, Math.min(max, zoom)) );
...
L.TileLayer = L.Class.extend({
	...
	_update: function (e) {
		...
		var nwTilePoint = new L.Point(
			...
			tileBounds = this._tileBounds(bounds);
	...
	getTileUrl: function (tilePoint, zoom) {
		...
		// console.log('getTileUrl ' + tilePoint + ' ' + this._getOffsetZoom(zoom) );
...
L.DomEvent = {
	...
	getMousePosition: function (e, container) {
		var x = e.pageX ? e.pageX : e.clientX +
			...
			pos = new L.Point(x, y);
		// mlog('getMousePosition ' + pos + ' offset ' + L.DomUtil.getViewportOffset(container) );
		return (container ?
...
L.Map.include(!(L.Transition && L.Transition.implemented()) ? {} : {
	setView: function (center, zoom, forceReset) {
		// console.log('actually its this setView');
